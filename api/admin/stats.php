...
try {
    ...
    if ($conn) $conn->close();
    echo json_encode(['success' => false, 'error' => 'Database exception: ' . $e->getMessage(), 'stats' => []]);
    exit;
}
...
if (!$connected || $conn->connect_error) {
    ...
    echo json_encode(['success' => false, 'error' => 'Database connection failed: ' . $conn->connect_error, 'stats' => []]);
    exit;
}
